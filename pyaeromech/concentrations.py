...
class SquareShoulderFilletRectangularSection(StressConcentration):
    
    def __init__(self, h, r, D, L, loadtype) -> None:
        super().__init__()
        self.r = r
        self.h = h
        self.D = D
    ...
    def factors(self, L, D, r, h):
        ratio = h/r
        if L/D < 3/((r/(D-2*h))**(1/4)):
            raise("The ratio L/D is outside the valid limits")
        
        if self.loadtype == "axial_tension":
            if 0.1 <= ratio <= 2.0:
                self.C = np.array([[1.007, 1.000, -0.031],
                                [-0.114, -0.585, 0.314],
                                [0.241, -0.992, -0.271],
                                [-0.134, 0.577, -0.012]])
            elif 2.0 <= ratio <= 20.0:
                self.C = np.array([[1.042, 0.982, -0.036],
                                [-0.074, -0.156, -0.010],
                                [-3.418, 1.220, -0.005],
                                [3.450, -2.046, 0.051]])
        
        elif self.loadtype == "in_plane_bending":
            if 0.1 <= ratio <= 2.0:
                self.C = np.array([[1.007, 1.000, -0.031],
                                [-0.270, -2.404, 0.749],
                                [0.677, 1.133, -0.904],
...
class TwoUNotchesRectangularSection(StressConcentration):
    
    def __init__(self, h, r, D, loadtype) -> None:
        super().__init__()
        self.r = r
        self.h = h
        self.D = D
        self.loadtype = loadtype
        self.combinations = np.meshgrid(r, h, D)
        results = np.empty([4, len(np.matrix.flatten(self.combinations[0]))])

        for idx, (ri, hi, Di) in enumerate(zip(np.matrix.flatten(self.combinations[0]), np.matrix.flatten(self.combinations[1]), np.matrix.flatten(self.combinations[2]))):
            C = self.factors(ri, hi)
            Kti = self.Kt(hi, ri, Di, C)
            results[0, idx] = ri
            results[1, idx] = hi
            results[2, idx] = Di
            results[3, idx] = Kti
    
    def factors(self, r, h):
        ratio = h/r
        ...
        elif self.loadtype == "out_of_plane_bending":
            if ratio == 1:
...
class SingleUNotchRectangularSection(StressConcentration):
    ...
    def __init__(self, h, r, D, loadtype) -> None:
        ...
        if loadtype == "axial_tension":
            if ratio == 1:
                self.C = np.array([2.988, -7.300, 9.742, -4.429])
            elif 0.5 <= ratio <= 4.0:
                self.C = np.array([[0.721, 2.394, -0.127],
                                [1.978, -11.489, 2.211],
                                [-4.413, 18.751, -4.596],
                                [2.714, -9.655, 2.512]])
            ...
            if ratio == 1:
                self.C = np.array([2.988, -7.735, 10.674, -4.927])
            elif 0.5 <= ratio <= 4.0:
                self.C = np.array([[0.721, 2.394, -0.127],
                                [-0.426, -8.827, 1.518],
                                [2.161, 10.968, -2.455],
...
class SingleVNotchRectangularSection(StressConcentration):
    
    def __init__(self, h, r, D, theta) -> None:
        self.r = r
        self.h = h
        self.D = D
        ratio = 2*h/D
        u_notch_reference = SingleUNotchRectangularSection(h,r,D, loadtype="in_plane_bending")
        
        Ktu = u_notch_reference.Kt()
        if theta <= 150:
...
class TwoVNotchesRectangularSection(StressConcentration):
    ...
    def __init__(self, h, r, D, theta) -> None:
        ...
        ratio = 2*h/D
        u_notch_reference = TwoUNotchesRectangularSection(h,r,D, loadtype="axial_tension")
